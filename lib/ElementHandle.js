...
class ElementHandle extends JSHandle {
  ...
  async _clickablePoint() {
    ...
    if (!result || !result.quads.length) {
      const error = new Error('Node is either not visible or not an HTMLElement');
      error.name = 'InvalidNodeError';
      throw error;
    }
  ...
  async $eval(selector, pageFunction, ...args) {
    ...
    if (!elementHandle) {
      const error = new Error(`Error: failed to find element matching selector "${selector}"`);
      error.name = 'ElementNotMatchedSelectorError';
      throw error;
    }
