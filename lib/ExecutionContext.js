...
class ExecutionContext {
  ...
  async evaluateHandle(pageFunction, ...args) {
    ...
    if (helper.isString(pageFunction)) {
      ...
      if (exceptionDetails) {
        const error = new Error('Evaluation failed: ' + helper.getExceptionMessage(exceptionDetails));
    ...
    if (typeof pageFunction !== 'function') {
      const error = new Error('The following is not a function: ' + pageFunction);
      error.name = 'InvalidFunctionError';
      throw error;
    }
    ...
    function convertArgument(arg) {
      ...
      if (objectHandle) {
        if (objectHandle._context !== this) {
          const error = new Error('JSHandles can be evaluated only in the context they were created!');
          error.name = 'JSHandleError';
          throw error;
        }
        if (objectHandle._disposed) {
          const error = new Error('JSHandle is disposed!');
          error.name = 'JSHandleError';
          throw error;
        }
