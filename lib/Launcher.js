...
function waitForWSEndpoint(chromeProcess, timeout) {
  return new Promise((resolve, reject) => {
    ...
    function onClose(error) {
      ...
      err.name = 'FailureToLaunchChromeError';
      reject(err);
    }

    function onTimeout() {
      cleanup();
      const error = new Error(`Timed out after ${timeout} ms while trying to connect to Chrome! The only Chrome revision guaranteed to work is r${ChromiumRevision}`);
