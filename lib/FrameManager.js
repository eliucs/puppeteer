...
class Frame {
  ...
  async addScriptTag(options) {
    if (typeof options.url === 'string') {
      ...
      try {
        ...
        const err = new Error(`Loading script from ${url} failed`);
        err.name = 'LoadingScriptError';
        throw err;
      }
    }
    ...
    const error = new Error('Provide an object with a `url`, `path` or `content` property');
  ...
  async addStyleTag(options) {
    if (typeof options.url === 'string') {
      ...
      try {
        ...
        const err = new Error(`Loading style from ${url} failed`);
        err.name = 'StyleLoadingError';
        throw error;
      }
    }
  ...
  */
  select(selector, ...values){
    ...
    return this.$eval(selector, (element, values) => {
      if (element.nodeName.toLowerCase() !== 'select') {
        const error = new Error('Element is not a <select> element.');
  ...
  waitFor(selectorOrFunctionOrTimeout, options = {}, ...args) {
    ...
    const error = new Error('Unsupported target type: ' + (typeof selectorOrFunctionOrTimeout));
    error.name = 'UnsupportedTargetTypeError';
  ...
  _detach() {
    for (const waitTask of this._waitTasks) {
      const error = new Error('waitForFunction failed: frame got detached.');
      error.name = 'WaitForFunctionFailedError';
      waitTask.terminate(error);
    }
    this._detached = true;
    if (this._parentFrame)
...
class WaitTask {
  ...
  constructor(frame, predicateBody, title, polling, timeout, ...args) {
    if (helper.isString(polling))
      ...
      const error = new Error('Unknown polling options: ' + polling);
      error.name = 'UnknownPollingOptionsError';
      throw error;
    }
    ...
    if (timeout) {
      ...
      timeoutError.name = 'TimeoutExceededError';
